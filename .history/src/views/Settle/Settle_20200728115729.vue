<template>
  <div>
    ...
    <div v-for="(item,index) in trueinfo" :key="index">
      <van-card :num="item.num" :price="item.present_price" :title="item.name" :thumb="item.image_path" />
    </div>
...
export default {
  ...
  mounted() {
    this.trueinfo=this.$route.query.trueinfo
    console.log(this.$route.query.trueinfo, "8888");
    ...
    this.pic = this.num * this.present_price * 100;
    this.image = this.arr.image;
    this.name = this.arr.name;
    this.id.push(this.arr.id);
    this.$rute.query.trueinfo;
