  ...
  <div>
    ...
    <van-tabs v-model="nane" style="margin-bottom:20px">
      ...
      <van-tab style="margin-bottom:50px">
        ...
        <div>
          <div v-for="(item1,index1) in this.msg" :key="index1" style="margin-top:20px">
            <div class="msgtop">
              <div>
                <div v-if="item1.anonymous === true" style="margin:0 0 0 10px">
                  <img :src="item1.comment_avatar" alt width="40px" class="imf" />
                </div>
                <div v-if="item1.anonymous === false" style="margin:0 0 0 10px">
                  <img :src="item1.user[0].avatar" alt width="40px" class="imf" />
                </div>
              </div>
              <div>
                ...
                <div style="margin-top:5px">
                  <van-rate v-model="item1.rate" :size="15" readonly />
                </div>
              </div>
              <div style="margin:0 10px 0 0">{{item1.comment_time}}</div>
            </div>
            <div style="margin:5px 0 0 90px">
              <div style="color:gray;font-size:8px">评论内容:</div>
              <div style="margin-top:10px">{{item1.content}}</div>
            </div>
          </div>
        </div>
      </van-tab>
    ...
    <van-goods-action style="background-color:white">
      ...
      <van-goods-action-icon icon="cart-o" text="购物车" @click="gotoshop" :badge="num1" v-if="this.user!=''" />
      <van-goods-action-icon icon="cart-o" text="购物车" @click="gotoshop"  v-if="this.user===''"/>
...
export default {
  ...
  methods: {
    ...
    //加入购物车
    addshop() {
      this.$utils.checkLogin(this.addshop1);
    },
    addshop1() {
      this.$api
        .addShop({
    ...
    //立即购买
    fast() {
      this.$utils.checkLogin(this.fast1);
    },
    fast1() {
      this.show = true;
    },
    ...
    //收藏商品
    Collection() {
      this.$utils.checkLogin(this.Collection1);
    },
    Collection1() {
      this.$api
    ...
    //取消收藏
     cancel() {
      this.$utils.checkLogin(this.cancel1);
    },
    cancel1() {
      this.$api
  ...
  mounted() {
    ...
    this.$api
      ...
      .then((res) => {
        ...
        let a = JSON.parse(localStorage.getItem("Recently_look"));
        if (a === null) {
          this.history = [];
          this.history.unshift(res.goods.goodsOne);
          localStorage.setItem("Recently_look", JSON.stringify(this.history));
        } else {
          this.history = [];
          console.log(a, "a");
          a.map((item, index) => {
            if (item.id === res.goods.goodsOne.id) {
              a.splice(index, 1);
            }
          });
          this.history = a;
