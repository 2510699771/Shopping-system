<template>
  <div class="box">
    ...
    <div class="thiscity" style="background-color:white;">{{thiscity}}</div>
    ...
    <van-index-bar :index-list="kind"  highlight-color="red">
      <div v-for="(item,index) in kind " :key="index">
        <van-index-anchor :index="item" sticky style="width:95%;" class="boo">{{item}}</van-index-anchor>
        <div v-for="(item1,index1) in citys[index]" :key="index1" class="buttom">
          <van-cell :title="item1.name" @click="allcity(item1,index1)" style="width:95%" />
        </div>
      </div>  
    </van-index-bar>
  </div>
...
export default {
  ...
  methods: {
    allcity(item1, index1) {
      ...
      localStorage.setItem("city", JSON.stringify(item1.name));
      this.$router.push("/mall");
    },
    //点击热门城市
    hot(item, index) {
      console.log(item, "item");
      localStorage.setItem("city", JSON.stringify(item.name));
      this.$router.push("/mall");
    },
    //返回
    ret() {
