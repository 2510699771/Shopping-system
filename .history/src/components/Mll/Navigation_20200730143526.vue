    ...
    <div>
      ...
      <div v-if="$store.state.rrr===0" class="app">
        <div v-for="(item,index) in goods" :key="index">
          <div><div v-html="item.name"   @click="shop(item,index)"></div></div>
        ...
        this.$api
          ...
          .then((res) => {
            ...
            res.data.list.map((item, index) => {
              ...
              // this.list[i].valueBright = this.list[i].value.replace(
              //   replaceReg,
              //   `<span style="color:#e5242b;height:40px;">${this.keyword.toUpperCase()}</span>`
              // );
            });
          })
          .catch();
